  ...
  <sch:pattern>
   ...
   -->
    ...
    <sch:rule context="oscal:param/oscal:value">
      <sch:assert role="warning" test="not(. = ../oscal:label)">Parameter value echoes description.</sch:assert>
    </sch:rule>
    ...
    <sqf:fixes>
      ...
      <sqf:fix id="add-new-param">
        ...
        <xsl:variable name="home" select="ancestor::oscal:control[1]"/>
        <xsl:variable name="id_guess">
          <xsl:for-each select="ancestor::oscal:control">
            <xsl:value-of select="(@id,oscal:prop[@name=('number','name','id')])[1]"/>
        ...
        <sqf:add use-when="exists($home/oscal:param)" position="after" match="ancestor::oscal:control[1]/oscal:param[last()]">
          <param id="{$param_id}">
            <xsl:copy-of select="$me/node()"/>
            <value><xsl:value-of select="$param_value"/></value>
          </param>
